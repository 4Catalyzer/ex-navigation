...
class ExNavigationBackButtonManager {
  ...
  disable() {
    this._listeners.forEach(listener => BackHandler.removeEventListener('hardwareBackPress', listener));
    BackHandler.addEventListener('hardwareBackPress', this._disabledBackButtonPress); // Don't let app be exited.
  }
  ...
  _setListeners(newListeners: Array<() => Promise<void>>) {
    ...
    BackHandler.removeEventListener('hardwareBackPress', this._disabledBackButtonPress);
    BackHandler.addEventListener('hardwareBackPress', this._listeners[this._listeners.length - 1]);
  }
